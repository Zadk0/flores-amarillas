...
(function() {
  ...
  // ==========================================
  // GALAXIA ESPIRAL DORADA (POLVO ESTELAR)
  // ==========================================
  function createGalaxy() {
    ...
    const colorInside = new THREE.Color(0xfffbeb); // Blanco cálido central
    const colorMid = new THREE.Color(0xfbbf24);    // Amarillo dorado
    const colorArm = new THREE.Color(0xd97706);    // Ámbar profundo
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      // Gradiente de color según distancia al centro
  ...
  // ==========================================
  // CONSTRUCCIÓN PROCEDURAL DE FLORES AMARILLAS 3D
  // ==========================================
  function createSingleFlower(scale = 1.0, petalCount = 20, isCentral = false) {
    const flowerGroup = new THREE.Group();

    // 1. Centro del girasol (disco con textura y semillas)
    ...
    // Puntos de semillas en espiral de Fibonacci en el centro
    ...
    // 2. Geometría de pétalo amarillo curvo
    ...
    // Material de pétalos amarillos brillantes
    ...
    for (let p = 0; p < petalCount; p++) {
      ...
      petal.rotation.x = 0.15; // Ligera inclinación hacia afuera
      flowerGroup.add(petal);
    }

    // Fila interior de pétalos (alternada para volumen frondoso)
    const innerCount = petalCount;
    for (let p = 0; p < innerCount; p++) {
      const angle = ((p + 0.5) / innerCount) * Math.PI * 2;
      ...
      petal.rotation.y = -angle + Math.PI / 2;
      petal.rotation.x = 0.25; // Más curvada hacia arriba
      flowerGroup.add(petal);
    }

    // Tallo y hojas verdes elegantes
    ...
    // Resplandor áurico central si es la flor principal
    if (isCentral) {
  ...
  function createFlowers() {
    // 1. Flor Central Majestuosa (El girasol central principal)
    centralFlowerGroup = createSingleFlower(1.8, 26, true);
    centralFlowerGroup.position.set(0, 2, 0);
    centralFlowerGroup.rotation.x = 0.35; // Mirando suavemente a la cámara
    scene.add(centralFlowerGroup);

    // Guardar para raycasting / interacción al hacer clic en el centro
    centralFlowerGroup.userData = { isCenter: true };
    interactiveObjects.push(centralFlowerGroup);

    // 2. Flores Amarillas Orbitando alrededor
